...
<head>
    ...
    <title>Crear Plantilla - Passiflor Admin</title>
...
<body>
    ...
    <div class="dashboard-wrapper">
        ...
        <div class="form-section therapy-form-section">
            <form action="{{ route('admin.therapies.store') }}" method="POST" enctype="multipart/form-data" class="therapy-form">
            <form action="{{ route('admin.therapies.store') }}" method="POST" enctype="multipart/form-data" class="therapy-form">
                @csrf
                ...
                <div class="form-group full-width">
                    ...
                    <input type="text" name="slug" value="{{ old('slug') }}" class="form-input" required placeholder="ej: entrenamiento-ir-bano">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <select name="therapist_id" class="form-input" {{ auth()->user()->actingAsTherapist() ? 'disabled' : '' }}>
                            ...
                            @foreach($therapists as $t)
                                <option value="{{ $t->id }}" {{ (auth()->user()->actingAsTherapist() && $t->id === auth()->id()) ? 'selected' : '' }}>
                                    {{ $t->name }} &lt;{{ $t->email }}&gt;
                                </option>
                            @endforeach
                        </select>
                        @if(auth()->user()->actingAsTherapist())
                            ...
                            <span class="form-hint">En modo terapeuta, solo puedes crear plantillas para ti.</span>
                        @endif
                    </div>
                    ...
                    <div class="form-group" id="patient-select-wrapper" style="display:none;">
                        ...
                        <select name="assigned_patient_id" id="assigned_patient_id" class="form-input" disabled>
                            <option value="">-- Sin paciente --</option>
                            @foreach($patients as $p)
                                <option value="{{ $p->id }}" {{ (old('assigned_patient_id') == $p->id) ? 'selected' : '' }}>{{ $p->name }} &lt;{{ $p->email }}&gt;</option>
                            @endforeach
                        </select>
                        @if(auth()->user()->actingAsTherapist())
                ...
                <div class="form-section-header">
                    <h3 class="form-section-title"><i class="ph ph-text-align-left"></i> Contenido de la Portada</h3>
                </div>
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        <input type="text" name="title" value="{{ old('title') }}" class="form-input" required placeholder="Ej: Entrenamiento para ir al baño">
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="subtitle" value="{{ old('subtitle') }}" class="form-input" placeholder="Ej: Planificación terapéutica">
                    </div>
                </div>
                ...
                <div class="form-section-header">
                    ...
                    <p class="form-section-description">Agrega páginas de tipo Step (listas con viñetas) o Info (párrafos de texto).</p>
                </div>
    ...
    <template id="page-template">
        <div class="page-item">
            <div class="page-item-header">
                ...
                <button type="button" class="remove-page btn-action btn-delete btn-icon">
    ...
    <script>
        ...
        function setupListItemControls(pageItem, idx) {
            ...
            // Setup remove for initial item
            pageItem.querySelector('.remove-list-item').addEventListener('click', (e) => {
        ...
        addPageBtn.addEventListener('click', () => {
            ...
            // Update all input names with the current index
                ...
                if (name) {
                    // Replace pages[][] with pages[index][field]
            ...
            setupPageTypeToggle(addedPage, pageIndex);
            setupListItemControls(addedPage, pageIndex);
        ...
        // Show patient select only after therapist is chosen (same behavior as edit)
